	...
	public static void main(String[] args) {
        ...
        try {
        	...
        	while (true) {
                ...
                int punctuationCount = processor.countPunctuationMarks();
                
	            byte[] outData = processor.convertToByteArray(totalCharacters);
	            byte[] vowelCountData = processor.convertToByteArray(vowelCount);
                byte[] consonantCountData = processor.convertToByteArray(consonantCount);
                byte[] punctuationCountData = processor.convertToByteArray(punctuationCount);
	            ...
	            DatagramPacket vowelCountPacket = 
	            		new DatagramPacket(vowelCountData, vowelCountData.length, clientAddress, clientPort);
                DatagramPacket consonantCountPacket = 
                		new DatagramPacket(consonantCountData, consonantCountData.length, clientAddress, clientPort);
                DatagramPacket punctuationCountPacket = 
                		new DatagramPacket(punctuationCountData, punctuationCountData.length, clientAddress, clientPort);
	            
	            // 9. Reply to client
	            datagramSocket.send(outPacket);
	            datagramSocket.send(vowelCountPacket);
                datagramSocket.send(consonantCountPacket);
                datagramSocket.send(punctuationCountPacket);
