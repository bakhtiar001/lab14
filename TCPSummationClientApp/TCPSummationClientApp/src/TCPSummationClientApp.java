...
public class TCPSummationClientApp {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 4. Process response from the server (get total and multi)
			InputStream inStream = socket.getInputStream();
			DataInputStream dis = new DataInputStream(inStream);
			int result = dis.readInt();
			int result2 = dis.readInt();
			
			// 5. Further processing - display the result of total and multi
			System.out.println("\tSending to the server: " + number1 + ", " 
					+ number2 + ", " + number3);
			System.out.println("\tReceiving from server: " + result + ", " + result2);
